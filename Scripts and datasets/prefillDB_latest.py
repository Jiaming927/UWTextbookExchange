...
while i < len(lines):
	...
	# Not book name line
		...
		if result is None:
			...
			templist = []
			cur.execute("SELECT course_name " + bookQuery + " FROM courses WHERE course_name = ?", (course_name,))
			result = cur.fetchone()
			for thing in result:
			...
			for book in books:
				if book not in bookSet:
					...
					cur.execute("UPDATE courses SET " + bookIndex + "=\'" + book + "\' " + "WHERE course_name=\'" + result[0] + "\'")		
					print("ADDED " + bookIndex + ": " + book + " to " + result[0])
